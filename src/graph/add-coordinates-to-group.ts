...
export default function addCoordinatesToGroup(
  ...
  return groups.map<Group>((group) => {
    ...
    return {
      ...
      center: [groupCenters[id].x, groupCenters[id].y],
...
function getCentroids(nodes: Node[]) {
  ...
  const centroids = Object.entries(groupCoordinates).reduce<
    Record<string, Coordinate>
  >((centroids, [group, coordinates]) => {
...
function getGroupCoordinates(nodes: Node[]) {
  return nodes.reduce<Record<string, CoordinatePair[]>>(
    (groupCoordinates, node) => {
