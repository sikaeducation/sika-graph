...
export default function getSmoothHull(
...
function createTwoPointHull([p1, p2]: CoordinatePair[], hullPadding: number) {
  ...
  const controlDelta = scaleVector([-vector[1], vector[0]], tangentHalfLength);
...
function createVector(
  ...
  return scaleVector(vector, length);
}
...
function expandPoint(hullPadding: number) {
  return (point: Vector, index: number, points: Vector[]): CoordinatePair => {
    ...
    const hullVector = sumVectors(previousPoint.v, scaleVector(point.v, -1));

    return sumVectors(point.p, scaleVector(hullVector, hullPadding));
  };
}
